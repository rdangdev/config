...
-- Beautiful theme
-- beautiful.init(home .. "/.config/awesome/oof.lua")
...
mailwidget:buttons(awful.util.table.join(
  awful.button({ }, 1, function () exec("mbsync -a 2>/dev/null") end)
))
-- }}}
...
-- Configure widget
local orgmode = {
  files = { home.."/Desktop/todo.org",
    home.."/Desktop/todo.org", home.."/Desktop/todo.org",
  },
  color = {
...
datewidget:buttons(awful.util.table.join(
  awful.button({ }, 1, function () exec("st -e calc") end)
))
-- }}}
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "w", function () exec("firefox-developer-edition") end),
  ...
  --awful.key({}, "#235", function () exec("xset dpms force off") end),
    ...
    awful.key({ altkey }, "F4", function ()
        awful.prompt.run({ prompt = "Web: " }, promptbox[mouse.screen].widget,
            function (command)
                sexec("firefox-developer-edition-developer-edition 'http://yubnub.org/parser/parse?command="..command.."'")
...
awful.rules.rules = {
    ...
    { rule = { class = "firefox-developer-edition",  instance = "Navigator" },
